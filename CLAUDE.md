...
| PHP | 8.5系 | Ubuntu 24.04標準リポジトリにはPHP 8.5系が無いため、`packages.sury.org`を追加して導入する。導入するパッケージバージョンは`apt-cache madison php8.5`で確認する（環境構築手順書1.2.2参照）。拡張：mbstring, xml, curl, zip, pdo_mysql, bcmath, gd, intl, fileinfo |
...
**注意**：テストサーバー・開発環境とXserver（本番）とで、PHP・MariaDBともパッチバージョンが完全には一致していない（PHP：`packages.sury.org`が提供する8.5系最新版（2026-09-18時点で8.5.10） 対 8.5.4、MariaDB：10.11.19 対 10.11.18）。OS・導入経路が異なる（Ubuntu＋sury.org／Rocky Linux＋サーバーパネル）ため、そもそも同一パッチバージョンを揃えることは難しく、いずれも同じメジャー系列内の差異にとどまるため実用上の影響は小さいと考えられるが、既知の差異として明記しておく。
