...
| PHP | 8.5系 | Ubuntu 24.04標準リポジトリにはPHP 8.5系が無いため、`ppa:ondrej/php`を追加して導入する。導入するパッケージバージョンは`apt-cache madison php8.5`で確認する（環境構築手順書1.2.2参照）。拡張：mbstring, xml, curl, zip, pdo_mysql, bcmath, gd, intl, fileinfo |
