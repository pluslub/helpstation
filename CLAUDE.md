...
**注意**：テストサーバー・開発環境とXserver（本番）とで、PHP・MariaDBともパッチバージョンが完全には一致していない（PHP：8.5.4 対 8.5.9、MariaDB：10.11.19 対 10.11.18）。OSが異なる（Ubuntu／Rocky Linux）ため、そもそも同一のパッケージバージョンを揃えることは難しく、いずれも同じメジャー系列内の差異にとどまるため実用上の影響は小さいと考えられるが、既知の差異として明記しておく。
