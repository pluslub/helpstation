...
| is_am_off / is_pm_off | BOOLEAN | NOT NULL | 午前／午後休み |
| start_time / end_time | TIME | NULL可 | 始業・終業時刻 |
...
| default_staff_id | INT | FK → staff.id, NULL可 | デフォルト担当者 |
| wheelchair_required | BOOLEAN | NOT NULL | 車いすの要否（電動／手動の区別は車両側で管理） |
| interval_unit | ENUM(日, 週, 月) | NULL可 | 利用間隔の単位 |
| interval_value | INT | NULL可 | 利用間隔の数値 |
| fixed_period_start / fixed_period_end | DATE | NULL可 | 固定（定期訪問）の対象期間 |
| fixed_days_of_week | VARCHAR | NULL可 | 固定訪問の曜日（複数可、カンマ区切り等） |
| fixed_start_time / fixed_end_time | TIME | NULL可 | 固定訪問の時刻 |
...
| date | DATE | NOT NULL | |
| applied_start_time / applied_end_time | TIME | NULL可 | 社員が申請した内容 |
| applied_am_off / applied_pm_off | BOOLEAN | NOT NULL | 休みの設定（午前休／午後休。両方trueで終日休み） |
| applied_desired_off | BOOLEAN | NOT NULL | 希望休フラグ。休み自体の設定は上記am_off／pm_offで行い、本フラグはその休みが職員本人の希望によるものであることを示し、自動シフト調整（8.2項目3の自動仮登録等、システムによる自動的な変更）の対象から除外するために使用する。管理者による手動編集は本フラグの有無にかかわらず可能 |
| approval_flag | BOOLEAN | NOT NULL | 承認フラグ |
| admin_modified_flag | BOOLEAN | NOT NULL | 管理者が変更したか |
| modified_start_time / modified_end_time | TIME | NULL可 | 管理者変更後の始業・終業時刻 |
| modified_am_off / modified_pm_off | BOOLEAN | NOT NULL | 管理者変更後の午前休／午後休 |
...
| date | DATE | NOT NULL | |
| start_time / end_time | TIME | NOT NULL | 「開始時刻＋終了時刻」方式（要件定義書8.1項目14） |
| support_type_id | INT | FK → support_type.id | |
| vehicle_id | INT | FK → vehicle.id, NULL可 | 具体的な車両が割り当て済みの場合のみ設定。車不要・私用車の場合は常にNULL。介護専用車両／普通社用車を選択していても、担当者・管理者が具体的な車両を選定するまではNULLのまま（6.4.4参照） |
...
- **配車優先度・車両空き判定**（要件定義書6.4.4）：介護専用車両は電動／手動を区別せず合算台数で空き判定を行う。配車優先度による車両の再割当はシステムが自動で行うが、予約自体の日時・担当者調整は担当者が手動で行う。
