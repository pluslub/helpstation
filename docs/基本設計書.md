...
| fixed_period_start | DATE | NULL可 | 固定（定期訪問）の対象期間（開始） |
| fixed_period_end | DATE | NULL可 | 固定（定期訪問）の対象期間（終了） |
| fixed_start_time | TIME | NULL可 | 固定訪問の時刻（開始） |
| fixed_end_time | TIME | NULL可 | 固定訪問の時刻（終了） |
