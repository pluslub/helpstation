...
| 11 | 監査ログ閲覧画面 | 未確定 | 要件定義書8.2項目5（保留） |
...
要件定義書8.2に残る5件は、確定次第本書に反映する。特に以下は基本設計に直接影響するため優先的に解消する。

- 項目4：車両の点検中・故障中等の一時的な稼働停止状態の扱い（vehicleテーブルへのステータス追加要否）
- 項目5：監査ログ閲覧画面の要否・仕様
